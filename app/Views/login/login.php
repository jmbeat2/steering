...
$(document).ready(function () {
    $("#loginForm").submit(function (event) {
        ...
        var csrfName = $("input[name=csrf_test_name]").attr("name");  // Get CSRF token name
        var csrfHash = $("input[name=csrf_test_name]").val();  // Get CSRF token value

        $.ajax({
            url: "<?= site_url('Empusersc/authenticate') ?>",
            type: "POST",
            data: formData + "&" + csrfName + "=" + csrfHash,  // Append CSRF token
            dataType: "json",
